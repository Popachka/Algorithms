        ...
        for line in file:
            ...
            #Если это никакой информации, что мы обрабатываем матч не было, то обрабатываем запрос
                ...
                if 'Total goals for' in line: # Кол-во голов команды за все матчи
                    ...
                    print(total_goals)
                    output_file.write(str(total_goals) + '\n')
                elif 'Total goals by' in line:
                    print('Total goals by')
                    ...
                    if count_game_this_team == 0:
                        ...
                        print(round(total_goals / count_game_this_team,3))
                        output_file.write(str(total_goals / count_game_this_team) + '\n')

                elif 'Mean goals per game by' in line:
                    print('Mean goals per game by')
                    ...
                    if total_games_this_team == 0:
                        ...
                        output_file.write(str(total_goals / total_games_this_team) + '\n')
                elif 'Goals on minute' in line:
                    print('Goals on minute')
                    parts = line.split()
                    ...
                    print('Score opens by team')
                    team_name = line.split('by')[-1].strip()[1:-1]
                    total_first_goals = 0
                    for matchnumber,matchinfo in history.items():
                        if matchinfo['first_goal'][0] == team_name:
                            total_first_goals += 1
                    print(total_first_goals)
                    output_file.write(str(total_first_goals) + '\n')
                elif 'Score opens by' in line:
                    print('Score opens by player')
                    player_name = line.split('by')[-1].strip()
                    total_first_goals = 0
                    for matchnumber,matchinfo in history.items():
...
print('----')
print(count_goal_players['qP tEqYXtZvwUAas djK'])